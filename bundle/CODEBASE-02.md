...
def _commit(ctx, api_key, data):
    ...
    res = {"decision": "COMPLETENESS_SEALED", "score": 0, "complete_version": VERSION,
           ...
           "detail": "period=%s;kind=%s;root=%s;count=%d" % (period, kind, root, len(leaves))}
    audit_hash, block_index, seq = ctx["seal"](ev, res, now, api_key)
...
def _prove(ctx, api_key, data):
    ...
    if not period or not value:
        return {"error": "period_and_value_required"}, 400
...
def _periods(ctx, api_key):
    with ctx["lock"]:
        if api_key:
            rows = ctx["conn"].execute(
                "SELECT period,kind,root,leaf_count,committed,block_index FROM complete_commit "
                "WHERE api_key=? ORDER BY period_start DESC", (api_key,)).fetchall()
        else:
            rows = ctx["conn"].execute(
                "SELECT period,kind,root,leaf_count,committed,block_index FROM complete_commit "
                "ORDER BY period_start DESC").fetchall()
    return {"count": len(rows),
            "periods": [{"period": r[0], "kind": r[1], "root": r[2], "leaf_count": r[3],
                         "committed_at": _iso(r[4]), "block_index": r[5]} for r in rows],
...
def _root(ctx, api_key, data):
    ...
    if not row:
        return {"error": "not_committed", "period": period, "kind": kind}, 404
    _cid, root, count, committed, chain_hash, block_index, start, end = row
