...
def _seal(ctx, event, result):
    ...
    # Confirmed against the live server 19 Aug 2026: seal() is
    # seal(event, result, ts) and result must be a dict, not a JSON string.
    # The rest stay as fallbacks so this module still runs on another host.
    attempts = [
        ("seal(event, dict, ts_float)", lambda: fn(event, result, now)),
        ("seal(event, dict, ts_iso)", lambda: fn(event, result, iso)),
        ("seal(event, dict)", lambda: fn(event, result)),
        ("seal(event, json_string, ts_float)", lambda: fn(event, payload, now)),
        ...
        ("seal(event=, result=)", lambda: fn(event=event, result=result)),
    ]
