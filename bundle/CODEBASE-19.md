...
<div class="wrap">
  ...
  <div class="eyebrow">sebbi.pro · systems whitepaper · v6 · plain english</div>
  ...
  <div class="honest" style="margin-bottom:26px"><b>New in v6 (August 2026).</b> Five capabilities moved from design into code and are documented in sections 25 to 30: completeness and absence proofs, proof of erasure that does not break the chain, fork detection, black-box reproducibility, and cross-organisation lineage — plus a standalone offline verifier that checks all of them without contacting this platform at all. Sections 1 to 24 are unchanged in substance from v5.</div>
...
<em>TIME</em>          <s>Bitcoin anchoring</s>        the chain tip fixed in a ledger
  ...
  <p>Nothing in the design so far stopped an operator running two histories in parallel: serve chain A to a witness, chain B to an auditor. Both receive a valid tip. Both anchor it. Both verify perfectly against the copy they were given. Neither can tell, because there was no way to ask the question that would expose it — <i>is the tip you are holding actually an ancestor of the chain I serve today?</i></p>
  <p>That is the split-view attack. Witnessing does not stop it. Anchoring does not stop it, because two forks can both be anchored quite happily. It was the last place an operator could lie, and it is closed as of August 2026.</p>
  <h3>The mechanism</h3>
  <p>An ordered Merkle tree is maintained over every entry in the chain in write order, following <b>RFC 6962</b> — the Certificate Transparency standard — deliberately unmodified. Anyone who has implemented a CT verifier can point it at this and it works without new code; inventing a bespoke tree would mean nobody could check us without writing something first, which is the opposite of the point.</p>
  <p>Note this tree is in <b>write order</b>, while the completeness tree in section 25 is <b>sorted</b>. They answer different questions — one proves nothing was reordered, the other proves what is absent — and their roots will never match, which is stated in the published spec so nobody reports it as a fault.</p>
  <p>Two questions become answerable, by anyone, with no account:</p>
  <ul>
    <li><b>Ancestry.</b> Hand back any tip we ever served and we prove it still sits in the chain we serve today, at the same position, under the current root. If it does not, the response says so explicitly — and if we issued that tip, the holder now has evidence of a fork.</li>
    <li><b>Prefix.</b> Prove the log at size 21 is contained in the log at size 48: nothing inserted, nothing removed, nothing reordered, only appended. That is what "append-only" actually means, and until now it was a claim rather than something a stranger could check.</li>
  </ul>
  <p>A checkpoint route additionally seals the current size and root <b>into the chain itself</b>, so it is anchored with everything else. After that, serving a different history for that size contradicts a timestamped record of our own making.</p>
  <h3>Why it matters more than it sounds</h3>
  <p>Every party who has ever held one of our tips can check it, on a timer, forever, without our cooperation. We cannot show two faces to the network, because the moment any holder of any old tip runs the check, a fork stops being hidden and becomes arithmetic. This is the mechanism that made certificate authorities honest; it should have arrived in compliance years ago.</p>
  <p class="honest"><b>What this does not do.</b> A fork is only <b>detected</b> if somebody actually checks a tip they were given — the network has to do its half, which is why the route is public, free and automatable. If nobody keeps our old tips, there is nothing to check us against, so detection scales with how many witnesses retain history. And this proves our published chain is internally append-only and that a given tip belongs to it; it says nothing about whether an entry should have been written in the first place.</p>

  <h2 id="s28"><span class="n">28.</span>Reproducibility, without disclosing the rules</h2>
  <p>Every compliance platform can tell you what it decided. Ask one to re-run a decision from last year, from its sealed inputs, and produce the same verdict, and almost none can. Not because they will not — because their scoring runs through a model call, and model calls are not reproducible. Same inputs, different day, different answer. What they hold is a description of an event that can never be performed twice.</p>
  <p>Section 2 explains why this platform is arithmetic instead. This section makes that property testable by a stranger.</p>
  <h3>The scoring logic is never disclosed</h3>
  <p>Stated first because it is the constraint everything else here is built around: no route publishes the decision function's source, weights, thresholds, signal names or intermediate values. The only thing that leaves the building is a SHA-256 of the deployed source — one-way, and revealing nothing about what it hashes.</p>
  <p>Determinism is proved as a <b>black box</b> instead, and that is a stronger test rather than a weaker one. A published listing only shows what the code says. Repeated challenge shows what production actually does, over time, on inputs the vendor did not choose.</p>
  <h3>How a stranger tests us</h3>
  <p>Send any inputs you like to the challenge endpoint. The run is executed, sealed into the chain, and the verdict returned along with a fingerprint of your own inputs. Then send the identical inputs again — an hour later, a year later, from a different address, without telling us.</p>
  <p>Every run is sealed separately, anchored, and witnessed by parties we do not control, so neither the first answer nor the tenth can be quietly revised. The full history of every run of that exact input is public: how many times, when, what verdict, under which code fingerprint. <b>If the verdict ever moves while the code fingerprint has not, the engine is not deterministic and the challenger holds the proof.</b> If the fingerprint moved too, that is a dated policy change rather than a fault — which is exactly why the fingerprint is sealed alongside.</p>
  <p>A self-audit route additionally re-runs a sample of sealed historical decisions under the currently deployed code and publishes the reproduction rate. Divergences are counted and reported, never filtered out. A falling rate is the most useful thing that route can say.</p>
  <p class="honest"><b>The honest cost, which is real.</b> An open scoring oracle can be probed: feed it a thousand variations, watch the verdicts move, and a determined party can map the decision boundary without ever seeing the code. That is a genuine exposure and it is the price of this proof. It is mitigated rather than eliminated — repeat submissions of inputs already seen are never limited, because retesting is the entire point, while novel inputs are rate limited, because bulk novel inputs are how a boundary gets mapped rather than how a claim gets tested; and anyone systematically probing leaves a sealed trail while doing it. The trade is deliberate: a closed engine nobody can test is worth less than a testable one somebody might partially map. And reproducibility is not fairness — that the same inputs still yield the same output says nothing about whether the output was right.</p>

  <h2 id="s29"><span class="n">29.</span>Lineage: provenance across company boundaries</h2>
  <p>Every audit trail described so far stops at the edge of the company that wrote it.</p>
  <p>A lender holds a score. The score came from a scoring supplier, which used a model, which was trained on a data snapshot bought from someone else. Four organisations, four audit trails, none referencing the others. Ask what produced the outcome and you get four separate answers and no way to join them. Every value-chain rule written in the last three years assumes somebody can. Nobody can.</p>
  <p>The obvious remedy is a consortium ledger — everyone writing to one shared record, governed by someone. It fails on the first question anybody asks, which is who runs it, and it never gets built.</p>
  <h3>Why this needs no consortium</h3>
  <p>The pieces were already in place. A chain tip commits to everything sealed beneath it (§3). That tip is already handed to peers hourly and sealed into <b>their</b> chains (§5), which are anchored and witnessed in turn. So a receipt can already be walked up to a tip, and that tip already sits inside chains its issuer does not control. <b>The trust problem is solved before provenance is even mentioned.</b></p>
  <p>The only missing piece was sideways: a decision recording which receipts fed it, and which chain each came from. One field. The graph then composes itself, with no agreement between parties beyond each one sealing its own work and publishing a tip.</p>
  <p>Nobody has to opt into provenance. They opt into witnessing, which they already want, and provenance falls out of it.</p>
  <h3>What it gives you</h3>
  <ul>
    <li><b>Trace</b> — walk upstream from any decision: what produced it, hop by hop, across every company involved.</li>
    <li><b>Impact</b> — walk downstream. When a data provider retracts a snapshot, a model version turns out faulty, or an upstream decision is overturned, the first question anyone asks is which outputs were affected. Today that takes weeks of email and is never complete. Here it is a query that crosses company boundaries, and the answer is itself sealed and dated — so the scope of a recall can be shown to have been determined honestly rather than quietly narrowed to suit. That is Article 20 corrective action turned from a fire drill into a lookup.</li>
    <li><b>A portable receipt</b> — a self-contained document that travels attached to an output. Whoever receives it can establish, with no account and without contacting us, that the decision sits in a log that has not been rewritten, that the log is witnessed by parties we do not control, that its period declared its total before anyone asked, that the same inputs still reproduce the verdict, and what fed it upstream.</li>
  </ul>
  <h3>Verification without trusting anyone in the chain</h3>
  <p>Nothing here asserts that a remote hop is valid. Every node in a trace carries the exact routes a third party runs <b>against that party's own host</b> — an ancestry proof that the receipt is still there, a completeness check that nothing was omitted from its period, and the witness list showing who else holds that chain's tips. We never certify somebody else's link. An auditor verifies the whole graph without trusting us, the supplier, or anyone in between. Where two answers disagree, the disagreement is the finding.</p>
  <p class="honest"><b>What an edge is and is not.</b> An edge is a sealed, dated, non-repudiable <b>claim</b> by the declaring party that these inputs fed that decision. Sealing does not make the claim true; it removes the ability to revise it quietly afterwards, which is the part that matters when an outcome is disputed a year later. Declaring is voluntary — a party who declares nothing is not caught out by this, they are simply the point where somebody else's lineage goes dark, and their customer is the one who notices. A cross-chain hop can only be checked while the other party keeps their routes up; a dead peer leaves a visible stub rather than a silently resolved one. And the graph is empty until a second organisation declares its inputs: the mechanism exists, the network does not yet.</p>

  <h2 id="s30"><span class="n">30.</span>The offline verifier</h2>
  <p>Everything above can be checked through public endpoints. There is an obvious objection to that, and it is a fair one: those endpoints are ours. A proof you can only check with the prover's own online tool is not a proof, it is a reassurance.</p>
  <p>So the checking was moved out of our hands entirely. A single file, no dependencies, no network calls, that never contacts sebbi.pro or anything else. Save the proof documents you hold, run it on your own machine with the wifi off, and it does the arithmetic locally. It works in four years. It works if this company no longer exists.</p>
  <p>What it checks:</p>
  <ul>
    <li><b>Inclusion</b> — a record is inside a sealed period, against the sealed root.</li>
    <li><b>Absence</b> — the two neighbouring leaves verify and are adjacent, leaving nowhere for the queried record to be.</li>
    <li><b>Ancestry</b> — a tip you hold is still on the chain being served, at the same position.</li>
    <li><b>Prefix</b> — the log at one size is contained in the log at a later size, with nothing inserted, removed or reordered.</li>
    <li><b>Stability</b> — across a set of replay runs, identical inputs produced identical verdicts under an unchanged code fingerprint.</li>
  </ul>
  <p>It ships with a <b>self test</b> that builds trees internally across every tree size and leaf position, and confirms that tampered proofs and a forged prefix are rejected in every case. Run that first, so you are not taking the tool's word for anything either.</p>
  <p>The full hashing rules for both trees are published at the spec endpoints, so anyone can write their own verifier and ignore ours entirely. That is the point rather than a concession. An evidence layer whose proofs can only be checked by the party being checked has not moved the trust anywhere; it has just relocated it.</p>
  <p class="honest"><b>Scope:</b> the verifier checks the mathematics of the proofs it is given. It does not check that a sealed record was true, and it does not verify the Bitcoin anchor — that is a separate check against an OpenTimestamps client and a Bitcoin node, deliberately out of scope for a file with no dependencies, and better done independently in any case.</p>

  <hr>
  <footer>
    <p style="margin-top:20px"><a href="/">sebbi.pro</a> · <a href="/brain">Brain</a> · <a href="/seal">Seal</a> · <a href="/verify">Verify</a> · <a href="/signal-packs">Signal Packs</a> · <a href="/developers">Developers</a> · <a href="/contact">Contact</a></p>
    <p style="margin-top:8px;color:var(--faint)">Monop Content · Blyth, Northumberland, UK · v6 · 3 August 2026</p>
  </footer>
</div>
</body>
</html>
