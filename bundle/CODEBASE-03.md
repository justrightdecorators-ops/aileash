...
PUBLIC = {("GET", "trace"), ("GET", "decision"), ("GET", "spec")}
...
def _issue(ctx, api_key, data):
    ...
    res = {"decision": "AUTHORITY_GRANTED", "score": 0, "continuity_version": VERSION,
           ...
           "detail": "grant=%s;parent=%s;depth=%d;digest=%s"
                     % (grant_id, parent_id, depth, digest)}
    audit_hash, block_index, seq = ctx["seal"](ev, res, now, api_key)
