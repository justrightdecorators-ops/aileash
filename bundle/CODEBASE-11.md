...
def _do_seal(ctx, event):
    """Call ctx['seal'] whichever signature it has. Never swallow the reason."""
    seal = ctx["seal"]
    attempts = []
    for args in ((event,), (event, {"sealed": True}), (event, "register")):
        try:
            result = seal(*args)
