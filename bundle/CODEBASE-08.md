...
1008 lines, 42592 bytes

```python
"""
modules/peer.py  v1.2  --  signed peer submission (shared secret)
...
def _register(ctx, data):
    ...
    audit_hash, _bi, _rs, seal_error = _try_seal(
        ctx, {"module": "peer", "action": "register", "peer_id": peer_id},
        {"registered": True}, now)
...
def _rotate(ctx, data):
    ...
    audit_hash, _bi, _rs, seal_error = _try_seal(
        ctx, {"module": "peer", "action": "rotate", "peer_id": peer_id},
        {"rotated": True}, now)
...
def _set_status(ctx, data, status):
    ...
    audit_hash, _bi, _rs, seal_error = _try_seal(
        ctx, {"module": "peer", "action": status, "peer_id": peer_id},
        {"status": status}, _now())
