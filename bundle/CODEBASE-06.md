...
Everything below is machinery for making that sentence checkable.
  ...
  TEMPORAL      every ancestor must be valid at the instant of evaluation
                AND again at the instant of binding. A leaf inside its
                window under an expired parent is dead, and an ALLOW whose
                lineage was revoked or expired after it was issued is dead
                too (v1.6.0).
...
----------------------------
  ...
  - No implicit inheritance of unknown keys. A constraint the parent never
    ...
    is escalating.
  - No stale standing at bind (v1.6.0). An ALLOW is not a ticket. At
    confirm, every grant in the lineage is re-checked for revocation,
    expiry and integrity, and the lineage digest must still match the one
    evaluated. Standing established at evaluation must still hold at the
    moment the action binds.
...
-------------
  ...
  - The risk engine is consulted at exercise, not again at confirm. The
    bind-time re-check covers authority standing only.
...
def _standing_at_bind(ctx, grant_id, lineage_digest, now):
    """Re-establish present standing at the consequence boundary (v1.6.0).

    An ALLOW records what held when it was evaluated. It is not execution
    authority after a load-bearing condition has changed. So at confirm,
    every grant in the lineage is re-read and re-checked at this instant -
    integrity, revocation, validity window - and the lineage must still be
    the one that was evaluated. Any failure refuses the binding and names
    what changed. A change outside the lineage touches none of these checks
    and so does not refuse anything.

    Returns (problems, checked) where checked lists what was re-read.
    """
    problems = []
    checked = []
    chain, err = _walk(ctx, grant_id)
    if err:
        problems.append("standing lost since evaluation: " + err["detail"])
        return problems, checked
    for g in chain:
        item = {"grant": g["id"], "integrity": "ok", "revoked": None,
                "window": [_iso(g["not_before"]), _iso(g["not_after"])]}
        if _grant_digest(g) != g["digest"]:
            item["integrity"] = "FAILED"
            problems.append("standing lost since evaluation: grant %s no longer matches "
                            "the digest sealed at issue" % g["id"])
        rev = _revocation(ctx, g["id"])
        if rev:
            item["revoked"] = rev["revoked_at"]
            problems.append("standing lost since evaluation: grant %s was revoked at %s"
                            % (g["id"], rev["revoked_at"]))
        if now < g["not_before"]:
            problems.append("standing lost since evaluation: grant %s is not valid until %s"
                            % (g["id"], _iso(g["not_before"])))
        if now >= g["not_after"]:
            problems.append("standing lost since evaluation: grant %s expired at %s"
                            % (g["id"], _iso(g["not_after"])))
        checked.append(item)
    current = hashlib.sha256(
        EVAL_PREFIX + _canon([g["digest"] for g in chain]).encode("utf-8")).hexdigest()
    if lineage_digest and current != lineage_digest:
        problems.append("standing lost since evaluation: the lineage is not the one that "
                        "was evaluated")
    return problems, checked
...
def _revoke(ctx, api_key, data):
    ...
    return {"grant": grant_id, "revoked_at": _iso(now), "reason": reason,
            ...
            "effect": "Transitive. Every grant derived from this one stops evaluating, without "
                      "each descendant having to be found and revoked separately. From v1.6.0 "
                      "an ALLOW already issued under it can no longer bind either.",
...
def _confirm(ctx, api_key, data):
    ...
    From v1.6.0 standing is also re-established here. An ALLOW whose lineage
    has since been revoked, expired or altered does not bind, however recent
    the evaluation. A change outside the lineage does not affect binding.
    """
    ...
    # Standing at the consequence boundary. What held at evaluation is
    # evidence of what held then, not authority now.
    standing_problems, standing_checked = _standing_at_bind(ctx, grant_id, lineage_digest, now)
    problems.extend(standing_problems)
    ...
    res = {"decision": "EXECUTION_BOUND" if accepted else "EXECUTION_REJECTED", "score": 0,
           ...
           "lineage_digest": lineage_digest,
           "standing_rechecked_at_bind": True,
           "standing_lost": bool(standing_problems),
           "detail": "eval=%s;bound=%s;standing_lost=%s;params=%s"
                     % (eval_id, accepted, bool(standing_problems), actual)}
    audit_hash, block_index, seq = ctx["seal"](ev, res, now, api_key)
    ...
    return {"evaluation": eval_id, "bound": accepted, "problems": problems,
            ...
            "standing_at_bind": {"checked_at": _iso(now),
                                 "lineage": standing_checked,
                                 "held": not standing_problems},
            "sealed_in_chain": audit_hash, "block_index": block_index, "receipt_seq": seq,
...
def _spec():
    return {
        ...
        "invariants": {
            ...
            "standing_at_bind": "at confirm, every grant in the evaluated lineage is re-checked "
                                "for revocation, expiry and integrity, and the lineage must "
                                "still be the one evaluated. An ALLOW whose standing was "
                                "defeated after evaluation does not bind; a change outside the "
                                "lineage does not prevent binding",
