...
LOCATION
--------
Lives in: modules/x401_adapter.py
Imports:  from witness import handle (Same folder)
===============================================================================
"""
...
# Import core witness handle directly from witness.py in the same modules directory
try:
...
def extract_chain_identity(headers: Dict[str, str], default_name: str = "anon-agent") -> str:
    """
    Extracts the self-declared chain/peer name from standard HTTP header formats.
    """
...
def process_x401_observation(
    ...
    """
    Translates external HTTP parameters into witness.py's native observe schema
    and executes handle() directly in Python memory.
    """
    norm_headers = {k.lower(): str(v).strip() for k, v in headers.items()} if headers else {}
...
def witness_middleware_handler(
    ...
    """
    Middleware handler for HTTP routes.
    """
...
def build_x401_challenge_header(chain_name: str, tip: str) -> Dict[str, str]:
    """
    x401 challenge header generator.
    """
    challenge_payload = {
        "protocol": "x401",
