...
# =====================================================================
# SECTION 1: THE CORE AI.TXT FILE STANDARD & CRITICAL CRYPTO KEYS
# =====================================================================

AI_TXT_CONTENT = """# ai.txt - Verifiable AI Governance Manifest v1.0
Domain: sebbi.pro
Jurisdiction: GB
...
# The local genesis root block key for the un-fudgeable ledger
GLOBAL_SOVEREIGN_LEDGER = ["0" * 64]

# =====================================================================
# SECTION 2: PARSING & KEY INTEGRITY TESTING ENGINE
# =====================================================================
...
class AITxtValidationEngine:
    ...
    def compute_sha256(self, input_string: str) -> str:
        """Generates the unique cryptographic validation key hash."""
        return hashlib.sha256(input_string.encode('utf-8')).hexdigest()

    def evaluate_network_packet(self, headers: dict, payload_body: str, path: str) -> dict:
        """
        Inline Interception Loop. Evaluates inbound traffic signatures 
        against the exact constraints parsed out of the ai.txt manifest.
        """
        ...
        # --- KEY TEST 1: BOT EXTRATION SHIELD ---
        ...
        if any(bot in user_agent.lower() for bot in malicious_scraper_agents):
            outcome = "BLOCK"
            rationale = "LEGAL_BLOCK: Unauthorized scraping detected via automated user-agent fingerprint."
            http_status_code = 403

        # --- KEY TEST 2: FINANCIAL ANTI-FRAUD TESTING ---
        ...
        if outcome != "BLOCK" and account_id != "unverified_identity":
            ...
            if len(self.institutional_velocity_cache[account_id]) > self.velocity_max_limit:
                outcome = "BLOCK"
                rationale = "FRAUD_BLOCK: High-frequency transaction velocity anomaly detected over server root paths."
                http_status_code = 403

        # --- KEY TEST 3: MANDATORY ADOPTION HANDSHAKE ---
        if outcome == "ALLOW" and institutional_sector in ['banking', 'insurance', 'legal']:
            if client_verify_endpoint == "NONE":
                outcome = "CHALLENGE"
                rationale = "REGULATORY_CHALLENGE: Unverified node state. Access refused under ai.txt validation criteria."
                http_status_code = 428

        # --- KEY TEST 4: IMMUTABLE MERKLE KEY CHAIN BINDING ---
        event_record = {
...
# =====================================================================
# SECTION 3: NETWORK INTERFACE LAYER
# =====================================================================
...
class SebbiProductionServerGateway(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        # Serve the official, standardized ai.txt file structure directly from the root
        if self.path == "/ai.txt":
...
# =====================================================================
# SECTION 4: CONTAINER STARTUP CONTROL POINT
# =====================================================================
