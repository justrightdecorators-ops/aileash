...
(function(){
  ...
  // The payload these POST routes expect is not published, so rather than
  // guess once and report a failure that is really our own, try the shapes
  // they plausibly take and say which one was accepted.
  function postShapes(url, inner){
    ...
    var rejected = {};
    function go(i){
      if (i >= shapes.length) return Promise.resolve({ok:false, rejected:rejected});
      return jpost(url, shapes[i].body).then(function(r){
        if (!r.ok){ rejected[shapes[i].name] = r.body; return go(i + 1); }
        return {ok:true, shape:shapes[i].name, body:r.body, rejected:rejected};
      }).catch(function(e){
  ...
  function firstMessage(rejected){
    for (var k in rejected){
      if (Object.prototype.hasOwnProperty.call(rejected, k)){
        ...
        if (typeof b === "string") return b.slice(0, 120);
      }
    }
  ...
  var runners = {
    ...
    rule_binding: function(c){
      return postShapes(c.endpoint || "/x/rulebind/prove", PROBE).then(function(res){
        ...
        return Promise.all(strings.map(function(s){
          ...
          return {verdict:"INCONCLUSIVE",
                  why:"accepted the <b>" + res.shape + "</b> payload, but no string it returned " +
    ...
    reproducibility: function(c){
      return postShapes(c.endpoint || "/x/replay/challenge", PROBE).then(function(res){
        ...
        var body = (res.shape === "flat") ? PROBE
                 : (res.shape === "inputs") ? {inputs:PROBE} : {event:PROBE};
        return jpost(c.endpoint || "/x/replay/challenge", body).then(function(b){
            ...
            if (va === vb){
              return {verdict:"PASS",
                      why:"identical inputs submitted twice both returned <b>" + va + "</b> " +
                          "under one code fingerprint \u2014 determinism shown without " +
