...
(function(){
  ...
  var runners = {
    ...
    reproducibility: function(c){
      var inner = {amount: 4200, velocity_1h: 3, velocity_24h: 11, country: "GB",
                   device_id: "self-check", anomaly: 0, device_risk: 0};
      // The payload this module wants is not published, so rather than guess
      // once and report a failure that is really our own, try the shapes it
      // plausibly takes and say which one it accepted.
      var shapes = [{name:"flat", body:inner},
                    {name:"inputs", body:{inputs:inner}},
                    {name:"event", body:{event:inner}}];

      function attempt(i, rejected){
        if (i >= shapes.length){
          return Promise.resolve({verdict:"INCONCLUSIVE",
            why:"the endpoint is live but rejected every payload shape this runner knows. " +
                "Read the messages and publish the expected shape",
            detail:rejected});
        }
        return jpost("/x/replay/challenge", shapes[i].body).then(function(a){
          if (!a.ok){
            rejected[shapes[i].name] = a.body;
            return attempt(i + 1, rejected);
          }
          return jpost("/x/replay/challenge", shapes[i].body).then(function(b){
            return jget("/x/replay/fingerprint").then(function(f){
              ...
              if (!va || !vb){
                return {verdict:"INCONCLUSIVE",
                        why:"both runs were accepted under the <b>" + shapes[i].name +
                            "</b> shape but no verdict field came back to compare",
                        detail:{first:a.body, second:b.body}};
              }
              if (va === vb){
                return {verdict:"PASS",
                        why:"identical inputs submitted twice both returned <b>" + va +
                            "</b> under one code fingerprint \u2014 determinism shown without " +
                            "disclosing any scoring logic",
                        detail:{shape:shapes[i].name, fingerprint:f.body,
                                first:a.body, second:b.body}};
              }
              return {verdict:"FAIL",
          ...
          rejected[shapes[i].name] = e.message;
          return attempt(i + 1, rejected);
        });
      }
      return attempt(0, {});
