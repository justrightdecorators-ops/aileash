...
(function(){
  ...
  // trust was learned from a rejection on the rule-binding route, so it is
  // seeded here rather than rediscovered on every run. Routes that pass the
  // inputs straight to the scorer return a generic failure and cannot be
  // learned from.
  var PROBE = {amount: 4200, velocity_1h: 3, velocity_24h: 11, velocity_5m: 1,
               country: "GB", user_id: "self-check", device_id: "self-check",
               trust: 0.5, anomaly: 0, device_risk: 0, hour: 12, ip_risk: 0,
               merchant_risk: 0, new_device: 0};
