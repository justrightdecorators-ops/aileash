...
1297 lines, 56273 bytes

```python
"""
modules/integrity.py  v1.4  -  the AI Integrity Declaration, a public checker,
...
not by sebbi.pro. The register lists the latest sealed verdict per domain,
...
SELF = {
    ...
    "anchor": {
        ...
        "note": "Tips are stamped hourly. No confirmed proof of a current-chain tip is declared yet, so anchoring is not claimed.",
    },
    "oversight": {
    ...
    "known_gaps": [
        ...
        "Anchoring: tips are stamped to Bitcoin hourly, but no confirmed proof of a current-chain tip is declared yet.",
    ],
}
...
CACHE_SECONDS = 600
USER_AGENT = "sebbi-integrity-checker/1.4 (+https://sebbi.pro/x/integrity/status)"
...
def _seal_verdict(domain, out, ctx):
    """Seal a verdict into the chain, at most once a day per domain unless
    the verdict changes, and never more than VERDICT_MAX_PER_HOUR an hour."""
    ...
    with _seal_lock:
        prev = _sealed.get(domain)
        if prev and prev["fp"] == fp and now - prev["at"] < VERDICT_RESEAL_SECONDS:
            return dict(prev["ref"], sealed_now=False)
    ...
    if isinstance(res, (list, tuple)):
        ...
        h, idx = res, None
    ref = {"block_index": idx, "audit_hash": h,
           "check_block": ("https://sebbi.pro/x/walk/block?index=%s" % idx)
           if idx is not None else None}
    with _seal_lock:
        _sealed[domain] = {"fp": fp, "at": now, "ref": ref}
    return dict(ref, sealed_now=True)
