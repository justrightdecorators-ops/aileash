...
788 lines, 35359 bytes

```python
"""
modules/credits.py  v1.2.1
...
1.2.1: uses the Stripe secret key server.py already has (STRIPE_SECRET), and
  gives /c/stripe its own signing-secret variable so it no longer shares one
  with server.py's /stripe-webhook.

RAILWAY VARIABLES
  STRIPE_TOPUP_LINK       the Stripe payment link for top-ups
  CREDITS_WEBHOOK_SECRET  the signing secret of the /c/stripe endpoint in Stripe
                          (STRIPE_WEBHOOK_SECRET is also accepted)
  STRIPE_SECRET           your Stripe secret key - already set for server.py
                          (STRIPE_SECRET_KEY also works)
...
def _webhook_secrets():
    out = []
    for name in ("CREDITS_WEBHOOK_SECRET", "STRIPE_WEBHOOK_SECRET"):
...
def _verify_stripe(raw, sig_header, secret):
    if isinstance(secret, (list, tuple)):
        return any(_verify_stripe(raw, sig_header, one) for one in secret)
...
def _stripe_key():
    return (os.environ.get("STRIPE_SECRET_KEY", "").strip()
            or os.environ.get("STRIPE_SECRET", "").strip())
