...
# ==============================================================================
# AILEASH BRAIN v2.0 — Cryptographic Instruction Governance Layer
...
class BrainAuditChain:
    ...
    def _tip(self):
        with sqlite3.connect(self.db_path) as c:
            r=c.execute("SELECT block_hash FROM brain_log ORDER BY id DESC LIMIT 1").fetchone()
        return r[0] if r else "BRAIN_GENESIS_ANCHOR"

    def seal(self,instruction,instruction_hash,letter_sig,decision,reason,threat_category,risk_score):
        ts=time.time();prev=self._tip()
        ...
        with self.lock:
            with sqlite3.connect(self.db_path) as c:
                c.execute("""INSERT INTO brain_log
                    (ts,instruction,instruction_hash,letter_sig,decision,reason,threat_category,risk_score,prev_hash,block_hash)
                    VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (ts,instruction,instruction_hash,letter_sig,decision,reason,threat_category,risk_score,prev,block_hash))
                c.commit()
        return block_hash

    def verify(self):
        with sqlite3.connect(self.db_path) as c:
            rows=c.execute("SELECT instruction_hash,decision,risk_score,prev_hash,block_hash,ts FROM brain_log ORDER BY id ASC").fetchall()
        if not rows:return{"valid":True,"blocks":0,"message":"Empty chain"}
        prev="BRAIN_GENESIS_ANCHOR"
        for i,r in enumerate(rows):
            ...
            prev=r[4]
        return{"valid":True,"blocks":len(rows),"tip":rows[-1][4],"message":"Chain intact"}

    def recent(self,limit=20):
        with sqlite3.connect(self.db_path) as c:
            rows=c.execute("SELECT ts,instruction,decision,threat_category,risk_score,block_hash FROM brain_log ORDER BY id DESC LIMIT ?",(limit,)).fetchall()
        return[{"ts":r[0],"instruction":r[1],"decision":r[2],"threat_category":r[3],"risk_score":r[4],"block_hash":r[5]}for r in rows]
...
class BrainGovernor:
    ...
    def evaluate(self,instruction):
        ...
        if ih in ALLOWED_INSTRUCTIONS:
            bh=self.chain.seal(instruction,ih,ls,"ALLOW","explicit_allowlist","allowlist",0.0)
            return self._r("ALLOW","explicit_allowlist","allowlist",0.0,ih,ls,bh,start)
        ...
        for blocked_set,category in CATEGORY_SETS+[(self._custom_blocked,"custom")]:
            if ih in blocked_set:
                ...
                bh=self.chain.seal(instruction,ih,ls,"BLOCK",f"blocked_{category}",category,rs)
                return self._r("BLOCK",f"blocked_{category}",category,rs,ih,ls,bh,start)

        for word in norm.split():
            wh=hash_word(word)
            if wh in BLOCKED_WORDS or wh in self._custom_words:
                bh=self.chain.seal(instruction,ih,ls,"BLOCK",f"blocked_word:{word}","blocked_word",0.75)
                return self._r("BLOCK",f"blocked_word:{word}","blocked_word",0.75,ih,ls,bh,start)

        for pattern,category,weight in SUSPICIOUS_PATTERNS:
            if pattern.search(norm):
                bh=self.chain.seal(instruction,ih,ls,"BLOCK",f"pattern:{category}",category,weight)
                return self._r("BLOCK",f"pattern:{category}",category,weight,ih,ls,bh,start)

        bh=self.chain.seal(instruction,ih,ls,"ALLOW","no_violations","none",0.0)
        return self._r("ALLOW","no_violations","none",0.0,ih,ls,bh,start)

    def _r(self,decision,reason,threat_category,risk_score,ih,ls,bh,start):
        return{"decision":decision,"reason":reason,"threat_category":threat_category,
            "risk_score":round(risk_score,4),"instruction_hash":ih,
            "letter_signature":ls[:32]+"...","audit_hash":bh,
            "ms":round((time.time()-start)*1000,3)}

    def add_blocked_instruction(self,instruction):
        h=hash_instruction(instruction);self._custom_blocked.add(h);return h

    def add_blocked_word(self,word):
        h=hash_word(word);self._custom_words.add(h);return h
...
if __name__=="__main__":
    ...
    print("AILEASH BRAIN v2.0")
    print("="*80)
    for t in tests:
        r=brain.evaluate(t)
        s="✅ ALLOW" if r["decision"]=="ALLOW" else "🚨 BLOCK"
        print(f"{s} | {t[:42]:<42} | score:{r['risk_score']} | {r['ms']}ms | {r['audit_hash'][:12]}...")
