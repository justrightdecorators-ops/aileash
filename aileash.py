...
AILEASH CORE ENGINE
===================
Production-grade AI governance and transaction scoring.
...
REQUIRED_FIELDS = {
    ...
    "country", "device_id", "anomaly", "device_risk"
}
...
# ======================================================
# MEMORY WINDOWS (velocity tracking)
# ======================================================
...
def velocity(user_id):
    return {
        "60s": len(WINDOW_60S[user_id]),
        "5m":  len(WINDOW_5M[user_id]),
        "1h":  len(WINDOW_1H[user_id]),
    }
...
def compute_score(signals):
    ...
    # Trust (inverted — low trust = high risk)
    ...
    # Amount — log scale so £18k isn't same as £1k
    log_amount = math.log1p(signals["amount"]) / math.log1p(10000)
    score += min(log_amount, 1) * 0.15
    ...
    with _db_lock:
        _conn.execute("""
            INSERT INTO audit_log(
                ts, user_id, event_json,
                result_json, prev_hash, audit_hash
            )
            VALUES(?,?,?,?,?,?)
        """, (
            ts,
...
def govern(event):
    ...
    ts = now()  # single timestamp for consistency
    ...
    audit_hash = append_audit(event, result, ts)
...
if __name__ == "__main__":
    print("=" * 50)
    print(" AILEASH GOVERNANCE ENGINE")
    print("=" * 50)
    ...
    demo_event = {
        ...
        "amount":      18500,
        "country":     "RU",
        "device_id":   "server-eu-1",
        "anomaly":     0.72,
        "device_risk": 0.61,
    }

    result = govern(demo_event)
    ...
    print("\nSYSTEM STATUS:")
    print("✓ Trust Engine Active")
    print("✓ Audit Chain Active")
    print("✓ Behaviour Analysis Active")
    print("✓ Governance Runtime Active")
