...
2. Check RETURN_STYLE below matches what your router expects back.
"""
...
PEERS = [
    {
        ...
        "tip_url": "https://redflagaipro.com/api/witness/tip",
        "observe_url": "https://redflagaipro.com/api/witness/anchor",
    },
]
...
# Some routers want a plain dict back, some want (status_code, dict).
# "dict" or "tuple".
RETURN_STYLE = "dict"
...
def _http(url, payload=None):
    ...
    data = None
    headers = {"Accept": "application/json", "User-Agent": "aileash-mutual/1.0"}
...
def push_one(peer, tip, height):
    """Hand our tip to one peer so they record it."""
...
def pull_one(peer):
    """Take a peer's tip and seal it into our chain, via our own observe
    route so it is recorded exactly like any other witnessed tip."""
    ...
    if seal_status in (401, 403):
        out["hint"] = "our own observe route rejected us - add 'observe' to the PUBLIC set in modules/witness.py"
    return out
...
def do_push():
    ...
    return {
        ...
        "our_tip": tip,
        "our_height": height,
        "results": [push_one(peer, tip, height) for peer in PEERS],
    }


def do_pull():
    return {"ok": True, "results": [pull_one(peer) for peer in PEERS]}
