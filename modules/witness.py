...
- This module does not verify a peer's chain is internally valid. It records
  what they claimed and when. Verification is the peer's own endpoint to offer.
    ...
    POST /x/witness/observe             chain, tip - we seal their tip
...
def _observe(ctx, api_key, data):
    ...
    # Time the peer claims it sealed at. Accept an epoch number or an ISO
    # timestamp, under either field name, and carry on without it if it is
    # neither - it is supporting detail, not the evidence.
