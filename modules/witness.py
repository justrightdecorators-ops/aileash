...
def _peers(ctx, api_key):
    with ctx["lock"]:
        if api_key:
            ...
            rows = ctx["conn"].execute("SELECT peer,COUNT(*),MIN(observed),MAX(observed),COUNT(DISTINCT tip) FROM witness_log GROUP BY peer ORDER BY MAX(observed) DESC", (api_key,) if False else ()).fetchall()
    t = time.time()
    peers = []
...
def handle(method, action, data, api_key, ctx):
    ...
    if method == "POST":
        ...
        if action == "observe":
            return _observe(ctx, api_key, data)
    else:
        if action == "tip":
