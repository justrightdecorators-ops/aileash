"""
AILeash - Complete Production Backend v2.1
"""
...
def send_key_email(to_email: str, api_key: str):
    if not SMTP_HOST or not SMTP_USER:
        print(f"[AILeash] No SMTP config. Key for {to_email}: {api_key}")
        return
    try:
...
def check_and_increment(api_key: str, tier: str, usage_count: int):
    if tier == "free" and usage_count >= FREE_TIER_LIMIT:
        raise HTTPException(status_code=402, detail=f"Free tier limit of {FREE_TIER_LIMIT} requests reached. Upgrade at $99/month.")
    conn = get_db()
...
def write_audit(api_key, agent_id, action, score, decision, amount, country, anomaly, device_risk) -> str:
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO audit_log (ts,api_key,agent_id,action,score,decision,amount,country,anomaly,device_risk,entry_hash,prev_hash) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
...
def root():
    return {
        ...
        "version": "2.1.0",
        "status": "operational",
        "signup": "POST /signup  body: {email}",
        "govern": "POST /govern  header: x-api-key"
    }
...
def signup(req: SignupRequest):
    ...
    if existing:
        ...
        return {"message": "Key already exists. Resent to your email.", "tier": "free", "free_requests": FREE_TIER_LIMIT}
