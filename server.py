...
PAGE = open("static/index.html").read() if __import__("os").path.exists("static/index.html") else ""

@app.route("/")
def index():
    if PAGE:
        return PAGE, 200, {"Content-Type": "text/html; charset=utf-8"}
    return "<h1>AILeash</h1><p>Coming soon.</p>"
...
@app.route("/stats")
def stats():
    return jsonify({"total_licences": auth.total_keys(), "active": auth.total_paid(), "mrr_usd": auth.total_paid() * 99})
