...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/":
        ...
        elif path in("/verify","/identity","/notary","/dashboard","/pricing","/docs","/blog","/status","/about","/legal","/privacy","/terms"):
            ...
            with _db_lock:
                r=_conn.execute("SELECT id,ts,audit_hash FROM audit_log WHERE event_json LIKE ? ORDER BY id ASC LIMIT 1",("%"+ch+"%",)).fetchone()
            if r:send_json(self,{"verified":True,"block_index":r[0],"sealed_at":r[1],"seal":r[2]})
