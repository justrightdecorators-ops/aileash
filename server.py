...
<nav>
  ...
  <div style="font-size:12px;color:#666">by Monopcontent</div>
</nav>
...
<section>
  ...
  <div class="code">
    <pre><span class="key">POST</span> /govern
<span class="key">x-api-key:</span> your_key

{
  <span class="key">"action":</span>      <span class="val">"transfer"</span>,
  <span class="key">"amount":</span>      <span class="val">5000</span>,
  <span class="key">"device_risk":</span> <span class="val">0.2</span>,
  <span class="key">"anomaly":</span>     <span class="val">0.1</span>,
  <span class="key">"country":</span>     <span class="val">"US"</span>
}

<span class="res">Response:</span>
{
  <span class="key">"decision":</span> <span class="val">"ALLOW"</span>,
  <span class="key">"score":</span>    <span class="val">0.23</span>
}</pre>
  </div>
</section>
...
def audit_verify(x_api_key: str = Header(...)):
    get_key_record(x_api_key)
    conn = get_db()
    rows = conn.execute("SELECT * FROM audit_log WHERE api_key=? ORDER BY id ASC", (x_api_key,
