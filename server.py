...
import json,math,time,sqlite3,hashlib,threading,random,string,hmac,base64
...
def seal(event,result,ts,api_key=None):
    ...
    api_key is stored alongside the block for per-customer visibility -
    it is NOT part of the hash payload, so all existing blocks stay valid."""
    with _db_lock:
        ...
        _conn.execute("INSERT INTO audit_log(ts,user_id,event_json,result_json,prev_hash,audit_hash,api_key) VALUES(?,?,?,?,?,?,?)",(ts,event["user_id"],json.dumps(event),json.dumps(result),prev,h,api_key or ""))
        _conn.commit()
    return h
...
def govern(event,api_key=None):
    ...
    result["audit_hash"]=seal(event,result,ts,api_key)
    if api_key:
        inc_usage(api_key)
        if dec=="BLOCK":
...
<meta name="description" content="AI governance, child safety, speed and fraud detection. Free to integrate. Guardian is free forever for families. EU AI Act. Child Safety. Speed. Fraud Detection.">
...
<section class="hero">
  <div class="hero-inner">
    ...
    <p class="hero-sub">Four products. All free to integrate. <strong>Your users pay for protection.</strong> You set what they pay on AILeash, SonicBoom and Sentinel &mdash; we take 50p per device per month, you keep the rest. Guardian is free for every family, forever.</p>
    <div class="hero-btns">
...
<section class="products-sec" id="products">
  <div class="products-inner">
    ...
    <p class="sec-sub">Every product runs on the same engine. AILeash, SonicBoom and Sentinel are free to integrate, then 50p per device per month. Guardian is free for every family, always.</p>
    <div class="product-cards">
      ...
      <div class="pc pc-gu">
        <div class="pc-badge">Guardian &mdash; Child Safety &middot; Free</div>
        <h3>AILeash Guardian</h3>
        <p class="pc-desc">A free tool that helps children spot the warning signs of grooming and manipulation. Your child can check a message that feels off any time, get an instant plain-English assessment, and reach CEOP, Childline or 999 in one tap. Free for every family, always.</p>
        <div class="pc-features">
          <div class="pcf">Message check &mdash; instant assessment against known grooming and manipulation patterns</div>
          <div class="pcf">Parent dashboard &mdash; see what's been checked and flagged</div>
          <div class="pcf">Up to 10 children per account &mdash; each with their own Guardian code</div>
          <div class="pcf">One-tap access to CEOP, Childline and 999</div>
        </div>
        <div class="pc-price">Free</div>
        <div class="pc-price-label">for every child, every family, always</div>
        <a href="/guardian-app" class="btn-block btn-gu">Get Guardian Free &rarr;</a>
        <p class="pc-note">No card &middot; No trial &middot; No charge, ever</p>
      </div>
      <div class="pc pc-sb">
...
<footer>
  ...
  <div class="foot-bottom">
    <span>&copy; 2026 Monop Content &middot; Justin Antony Dobson &middot; Blyth, UK &middot; v6.1.0</span>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path in("/api/govern","/govern"):
            ...
            if not check_admin(self):send_json(self,{"error":"unauthorized"},401);return
            with _db_lock:
                rows=_conn.execute("SELECT ts,name,email,phone,
