...
# ---------- GUARDIAN (honest child-safety: check-in + panic + provable report) ----------
# Design rule: Guardian NEVER asserts a message is "safe". It flags known-risky
# patterns, and otherwise says "unrecognised - show a trusted adult". It stores
# only a fingerprint (hash) of any shared text, never the text itself.
GUARDIAN_PATTERNS=[
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/":
            ...
            send_html(self,'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><title>Guardian</title>\n<style>*{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}body{font-family:-apple-system,system-ui,sans-serif;background:#0a0f1e;color:#fff;min-height:100vh;padding:20px}\n.wrap{max-width:440px;margin:0 auto}h1{font-size:22px;margin:16px 0 4px;text-align:center}.sub{text-align:center;color:#8a90a6;font-size:13px;margin-bottom:24px}\n.big{display:block;width:100%;border:none;border-radius:16px;padding:26px;font-size:20px;font-weight:800;margin-bottom:14px;cursor:pointer;color:#fff}\n.help{background:#c8362b}.check{background:#1a9e6e}.flag{background:#c9a84c;color:#0a0f1e}\ntextarea{width:100%;border-radius:12px;border:1px solid #2a3350;background:#111a30;color:#fff;padding:14px;font-size:15px;min-height:90px;margin-bottom:12px}\n.out{border-radius:12px;padding:16px;margin:14px 0;font-size:15px;line-height:1.6;display:none}\n.out.flagged{background:#2a0f0d;border:1px solid #c8362b;display:block}.out.unrec{background:#1a1f10;border:1px solid #c9a84c;display:block}\n.row{display:flex;gap:10px;margin-top:20px}.row a{flex:1;text-align:center;background:#111a30;border:1px solid #2a3350;border-radius:12px;padding:14px;color:#fff;text-decoration:none;font-size:13px;font-weight:700}\n.tiny{text-align:center;color:#5a6178;font-size:11px;margin-top:20px;line-height:1.6}</style></head>\n<body><div class="wrap"><h1>Guardian</h1><div class="sub" id="who">Loading...</div>\n<button class="big help" onclick="panic()">I need help now</button>\n<button class="big check" onclick="checkin()">Check in - I\'m OK</button>\n<textarea id="msg" placeholder="Got a message that feels off? Paste it here and tap Check message."></textarea>\n<button class="big flag" onclick="flag()">Check this message</button>\n<div class="out" id="out"></div>\n<div class="row"><a href="tel:999">Call 999</a><a href="tel:08001111">Childline</a><a href="https://www.ceop.police.uk/safety-centre/" target="_blank">CEOP</a></div>\n<div class="tiny">Guardian never says a message is safe. It flags what it recognises. If anything feels wrong, trust that feeling and tell an adult you trust. Your messages are never stored.</div></div>\n<script>\nvar code=new URLSearchParams(location.search).get(\'code\')||\'\';\ndocument.getElementById(\'who\').textContent=code?(\'Linked. Your grown-up can see what you share.\'):\'No code - ask your parent for your link.\';\nfunction loc(cb){if(!navigator.geolocation){cb(null,null);return;}navigator.geolocation.getCurrentPosition(function(p){cb(p.coords.latitude,p.coords.longitude)},function(){cb(null,null)},{timeout:5000})}\nasync function post(u,b){try{var r=await fetch(u,{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify(b)});return await r.json()}catch(e){return{error:\'offline\'}}}\nfunction panic(){loc(function(la,lo){post(\'/api/guardian/panic\',{code:code,lat:la,lon:lo}).then(function(d){show(\'flagged\',\'Help is on the way. Your grown-up has been told. If you are in danger call 999 now.\')})})}\nfunction checkin(){loc(function(la,lo){post(\'/api/guardian/checkin\',{code:code,lat:la,lon:lo}).then(function(d){show(\'unrec\',\'Checked in. Nice one.\')})})}\nfunction flag(){var m=document.getElementById(\'msg\').value;if(!m){return}post(\'/api/guardian/flag\',{code:code,message:m}).then(function(d){document.getElementById(\'msg\').value=\'\';show(d.result===\'FLAGGED\'?\'flagged\':\'unrec\',d.advice)})}\nfunction show(cls,txt){var o=document.getElementById(\'out\');o.className=\'out \'+cls;o.textContent=txt}\n</script></body></html>')
        elif path=="/guardian-parent":
            send_html(self,'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Guardian - Parent</title>\n<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,system-ui,sans-serif;background:#0a0f1e;color:#fff;min-height:100vh;padding:20px}\n.wrap{max-width:600px;margin:0 auto}h1{font-size:24px;margin:12px 0}.sub{color:#8a90a6;font-size:14px;margin-bottom:20px}\ninput{width:100%;border-radius:10px;border:1px solid #2a3350;background:#111a30;color:#fff;padding:13px;font-size:15px;margin-bottom:10px}\nbutton{background:#c9a84c;color:#0a0f1e;border:none;border-radius:10px;padding:13px 20px;font-weight:800;cursor:pointer;font-size:15px}\n.card{background:#111a30;border:1px solid #2a3350;border-radius:12px;padding:18px;margin:12px 0}\n.ev{border-left:3px solid #2a3350;padding:10px 14px;margin:8px 0;font-size:14px}\n.ev.panic{border-color:#c8362b}.ev.flag{border-color:#c9a84c}.ev.checkin{border-color:#1a9e6e}\n.k{font-weight:700;text-transform:uppercase;font-size:11px;letter-spacing:1px}.panic .k{color:#e4574b}.flag .k{color:#c9a84c}.checkin .k{color:#7fe3b0}\n.t{color:#8a90a6;font-size:12px}a.map{color:#8fbcff}.seal{font-family:monospace;font-size:10px;color:#5a6178}\n.tiny{color:#5a6178;font-size:12px;margin-top:16px;line-height:1.6}</style></head>\n<body><div class="wrap"><h1>Guardian</h1><div class="sub">A provable record of what your child shares. Enter your API key to view.</div>\n<input id="key" placeholder="Your AILeash API key (al_live_...)"><input id="code" placeholder="Child pairing code (e.g. SEBBY-1A2B)">\n<button onclick="load()">View report</button>\n<div id="out"></div>\n<div class="tiny">Guardian shows only what your child chooses to share plus their check-ins and any messages they flag. It cannot secretly read their phone - and that is deliberate. Message content is never stored, only a tamper-evident fingerprint. Every event is sealed in the audit chain.</div></div>\n<script>\nasync function load(){var k=document.getElementById(\'key\').value.trim(),c=document.getElementById(\'code\').value.trim();if(!k||!c)return;\nvar r=await fetch(\'/api/guardian/report?code=\'+encodeURIComponent(c),{headers:{\'Authorization\':\'Bearer \'+k}});var d=await r.json();var o=document.getElementById(\'out\');\nif(d.error){o.innerHTML=\'<div class="card">Could not load: \'+d.error+\'</div>\';return}\nvar last=d.last_checkin?new Date(d.last_checkin*1000).toLocaleString():\'never\';\nvar h=\'<div class="card"><div class="k">\'+d.child+\'</div><div class="t">Last check-in: \'+last+\'</div></div>\';\nd.events.forEach(function(e){var when=new Date(e.ts*1000).toLocaleString();var extra=\'\';\nif(e.lat&&e.lon)extra+=\' <a class="map" href="https://maps.google.com/?q="+e.lat+","+e.lon+\'" target="_blank">map</a>\';\nif(e.note)extra+=\' - \'+e.note;\nh+=\'<div class="ev \'+e.kind+\'"><span class="k">\'+e.kind+\'</span> <span class="t">\'+when+\'</span>\'+extra+\'<br><span class="seal">sealed \'+e.sealed+\'</span></div>\'});\no.innerHTML=h}\n</script></body></html>')
            ...
            # parent pulls the provable timeline for one paired child
            auth=get_bearer(self)
            if not auth:send_json(self,{"error":"api_key_required"},401);return
            pc=qs.get("code",[""])[0].strip()
            with _db_lock:
                fam=_conn.execute("SELECT child_name,created,last_checkin FROM guardian_family WHERE pair_code=? AND parent_key=?",(pc,auth)).fetchone()
            ...
            if not auth:send_json(self,{"error":"api_key_required"},401);return
            with _db_lock:
                rows=_conn.execute("SELECT pair_code,child_name,last_checkin FROM guardian_family WHERE parent_key=? ORDER BY created ASC",(auth,)).fetchall()
    ...
    def do_POST(self):
        ...
        if path in("/api/govern","/govern"):
            ...
            if product not in("aileash","sonicboom","sentinel"):product="aileash"
            ...
            signup_msg="100 free decisions. After trial: \u00a3"+str(monthly)+"/month via Stripe."
            ...
            # parent creates a pairing code for a child (needs their API key)
            auth=get_bearer(self)
            if not auth:send_json(self,{"error":"api_key_required"},401);return
            ki=get_key(auth)
            if not ki:send_json(self,{"error":"invalid_api_key"},401);return
            ...
            with _db_lock:
                n=_conn.execute("SELECT COUNT(*) FROM guardian_family WHERE parent_key=?",(auth,)).fetchone()[0]
            if n>=10:send_json(self,{"error":"max 10 children per account"},400);return
            pc=gen_pair_code(nm)
            with _db_lock:
                _conn.execute("INSERT INTO guardian_family(pair_code,parent_key,child_name,created,last_checkin) VALUES(?,?,?,?,0)",(pc,auth,nm,now()))
                _conn.commit()
            ...
            # child device checks in (optionally with browser-provided location)
            ...
            # child taps "I need help" - highest priority, alerts parent
            pc=str(data.get("code","")).strip()
            with _db_lock:
            ...
            # child pastes a message that feels off - Guardian checks patterns, never says "safe"
            pc=str(data.get("code","")).strip()
            ...
            keys=[{"key":r[0],"email":r[1],"name":r[2],"org":r[3],"product":r[4],"devices":r[5],"actions_used
