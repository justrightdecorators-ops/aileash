...
def send_email(to_email,to_name,subject,html):
    ...
    try:
        req=urllib.request.Request("https://api.brevo.com/v3/smtp/email",
            data=json.dumps({"sender":{"name":"AILeash","email":"justrightdecorators@gmail.com"},"to":[{"email":to_email,"name":to_name}],"subject":subject,"htmlContent":html}).encode(),
...
async function chatSend(){
  ...
  try{
    ...
    var reply=data.content&&data.content[0]?data.content[0].text:'Sorry, email justin@monopcontent.com directly.';
    hideTyping();chatMsgs.push({role:'assistant',content:reply});addChatMsg('ai',reply);
  }catch(e){hideTyping();addChatMsg('ai','Having trouble connecting. Email justin@monopcontent.com or call 07908 269428.');}
