...
async function chatSend(){
  ...
  try{
    ...
    var reply=data.reply||'Sorry I could not get a response. Email justin@monopcontent.com directly.';
    hideTyping();
    ...
    def do_POST(self):
        ...
        if path in("/api/govern","/govern"):
            ...
            if not ANTHROPIC_KEY:
                send_json(self,{"reply":"I am not configured yet. Email justin@monopcontent.com or call 07908 269428."});return
            messages=data.get("messages",[])
            if not messages:send_json(self,{"reply":"No message received."},400);return
            CHAT_SYS="You are the AILeash sales and compliance assistant on sebbi.pro, built by Justin Antony Dobson at Monop Content in Blyth UK. You know everything about the four products: AILeash (AI governance EU AI Act Art 9/12/13/14 SHA-256 Merkle chain), AILeash Guardian (child safety grooming detection CSAM mandatory block Online Safety Act 2023), SonicBoom (speed plugin for existing cloud AI one line of code), AILeash Sentinel (fraud and anomaly monitoring real-time alerts). All products free to integrate. We take 50p per device per month. Platforms set their own user price and keep everything above 50p. Referral programme pays 10p per device per month forever. Be direct, helpful, plain English. Explain compliance like talking to someone who has never heard of it. Answer questions fully. When someone is ready direct them to sign up. If they want to contact Justin: justin@monopcontent.com or 07908 269428."
            try:
                req=urllib.request.Request("https://api.anthropic.com/v1/messages",
                    data=json.dumps({"model":"claude-sonnet-4-6","max_tokens":600,"system":CHAT_SYS,"messages":messages[-10:]}).encode(),
                    headers={"x-api-key":ANTHROPIC_KEY,"anthropic-version":"2023-06-01","Content-Type":"application/json"},method="POST")
                with urllib.request.urlopen(req,timeout=30) as r:
                    resp=json.loads(r.read())
                    reply=resp["content"][0]["text"] if resp.get("content") else "Sorry, no response. Email justin@monopcontent.com"
                send_json(self,{"reply":reply})
            except Exception as e:
                print(f"Chat error:{e}",flush=True)
                send_json(self,{"reply":"Having trouble right now. Email justin@monopcontent.com or call 07908 269428."})
