...
OWNER_EMAIL="hello@monopcontent.com"
    ...
    if(d.api_key){
      ...
      setTimeout(function(){
        fetch('/create-checkout',{method:'POST',headers:{'Content-Type':'application/json'},
          ...
          else{ok.textContent='Key ready. Justin will be in touch. hello@monopcontent.com';}
        }).catch(function(){ok.textContent='Key ready. Justin will be in touch. hello@monopcontent.com';});
      },2000);
    }else{err.textContent=d.error||'Something went wrong. Email hello@monopcontent.com';err.classList.add('show');btn.textContent=orig;btn.disabled=false;}
  }catch(e){err.textContent='Cannot reach server. Email hello@monopcontent.com';err.classList.add('show');btn.textContent=orig;btn.disabled=false;}
}
...
async function chatSend(){
  ...
  try{
    ...
    var reply=data.content&&data.content[0]?data.content[0].text:'Sorry, email hello@monopcontent.com directly.';
    hideTyping();chatMsgs.push({role:'assistant',content:reply});addChatMsg('ai',reply);
  }catch(e){hideTyping();addChatMsg('ai','Having trouble connecting. Email hello@monopcontent.com or call 07908 269428.');}
...
async function sendEmail(){
  ...
  try{
    ...
    else{status.textContent='Something went wrong. Email hello@monopcontent.com directly.';}
  }catch(e){status.textContent='Cannot reach server. Email hello@monopcontent.com directly.';}
}
</script>
</body>
</html>"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # ---- FUTURE ROUTES — stub, serve from HTML file if exists ----
