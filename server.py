...
def seal(event,result,ts):
    """Fix: tip read + hash + insert now happen inside ONE lock hold.
    Previously two concurrent requests could read the same tip and fork
    the chain, making verify_chain report false tampering."""
    with _db_lock:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path in("/api/govern","/govern"):
            ...
            send_json(self,{"api_key":key,"email":email,"product":product,"devices":devices,"monthly":monthly,"quota":FREE_QUOTA,"ref_code":new_ref_code,"endpoint":HOST+"/api/govern","message":signup_msg})
        elif path=="/contact":
            name=str(data.get("name","")).strip()
            email=str(data.get("email","")).strip().lower()
            phone=str(data.get("phone","")).strip()
